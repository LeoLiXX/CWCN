...
public final class CwRecording8ForcedReplayOpeningProbeTest {
    private static final long WINDOW_END_MS = 8000L;
    private static final int PREFERRED_TONE_HZ = 700;
    private static final int SEED_WPM = 15;
    private static final int SQL_PERCENT = 55;

    @Test
    public void printRecording8ForcedReplayOpeningComparison() throws Exception {
        ...
        LocalAudioDecodeTestSupport.ForcedToneReplayResult fixed650Replay =
                LocalAudioDecodeTestSupport.replayForcedConstantToneDecode(live, 650);
        LocalAudioDecodeTestSupport.ForcedToneReplayResult fixed660Replay =
                LocalAudioDecodeTestSupport.replayForcedConstantToneDecode(live, 660);
        ...
        System.out.println("==== recording8 forced replay opening comparison ====");
        System.out.println("expectedOpening=CQ CQ DE BG1XXX K BG1XXX ...");
        System.out.println(String.format(
                Locale.US,
                "live target=%d effective=%d representative=%d adopted=%d",
                live.probeResult().signalSnapshot().targetToneFrequencyHz(),
                live.probeResult().signalSnapshot().effectiveTrackedToneFrequencyHz(),
                live.probeResult().signalSnapshot().representativeLockedToneFrequencyHz(),
                live.probeResult().signalSnapshot().finalAdoptedFrequencyHz()
        ));
        ...
        printOpening("FIX650", fixed650Replay.decodeEvents(), fixed650Replay.timingEvents(), fixed650Replay.toneEvents());
        printOpening("FIX660", fixed660Replay.decodeEvents(), fixed660Replay.timingEvents(), fixed660Replay.toneEvents());
