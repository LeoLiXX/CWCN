...
public final class CwDirtyCase222505AbsoluteSqlProbeTest {
    ...
    private static final int[] SQL_LEVELS = new int[]{500, 606, 700, 900};

    @Test
    public void print222505AbsoluteSqlSweep() throws Exception {
        ...
        System.out.println("==== 20260427_222505 absolute sql sweep ====");
        for (int sqlLevel : SQL_LEVELS) {
            LocalAudioDecodeTestSupport.OfflineDetailedProbeResult detailed =
                    LocalAudioDecodeTestSupport.decodeFramesDetailedLiveLikeWithFrameHook(
                            SOURCE_LABEL + "-abs-sql-" + sqlLevel,
                            frames,
                            PREFERRED_TONE_HZ,
                            SEED_WPM,
                            55,
                            false,
                            (frame, signalProcessor, frameIndex) -> {
                                if (frameIndex == 0) {
    ...
    private static void printResult(
        ...
        System.out.println(String.format(
                Locale.US,
                "ABS sql=%d recall=%.3f tone=%d/%d/%d wpm=%d chars=%d thr=%d/%d on=%d off=%d src=%s/%s final=%s",
                sqlLevel,
                recall,
